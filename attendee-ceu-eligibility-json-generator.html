...
/* ---------- CEU PUBLISH ---------- */
document.getElementById('publishCEUBtn').addEventListener('click', () => {
  if (!lastOutputCEUObj) { // <-- must keep the raw JSON object
    $errs.textContent = 'Generate CEU JSON first.';
    return;
  }
  publishToGist(
    '/api/update-gist',
    ...
    (msg) => { $errs.textContent = msg; }
  );
});

/* ---------- SPEAKER (NO EMAIL) PUBLISH ---------- */
document.getElementById('publishSpeakersNoEmailBtn').addEventListener('click', () => {
  if (!lastOutputSpeakersNoEmailObj) { // <-- must keep raw JSON object too
    $errs.textContent = 'Generate Speakers JSON first.';
    return;
  }
  publishToGist(
    '/api/update-session-speaker-gist',
    lastOutputSpeakersNoEmailObj,
    () => {
